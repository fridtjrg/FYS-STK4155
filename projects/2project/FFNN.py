...
class Loss_function(Loss):
	def forward(self, y_pred, y_target): #Cost function?
		samples = len(y_pred)
...
class train():
	def backprop(selv, weights, loss, l, z):
		#Should be an elementwise multiplication(Hadamard)
		#check http://neuralnetworksanddeeplearning.com/chap2.html
		error = (weights[l+1]*loss[l+1])* sigmoid_prime(z[l])
