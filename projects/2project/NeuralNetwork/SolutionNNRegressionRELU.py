...
nn = NeuralNetwork(x_y_train, z_train, n_epochs = epochs, batch_size = batch_size)
nn.add_layer(DenseLayer(x_y_train.shape[1], n_hidden_neurons, 'relu', lmbda= Lambda[best_lambda_rate_NN], eta=Eta[best_learning_rate_NN]))
nn.add_layer(DenseLayer(n_hidden_neurons, n_hidden_neurons, 'relu', lmbda= Lambda[best_lambda_rate_NN], eta=Eta[best_learning_rate_NN]))
...
plt.savefig('../Figures/NN/train_heatmap_using_relu.pdf')
...
ax.set_ylabel("$\eta$")
ax.set_xlabel("$\lambda$")
plt.savefig('../Figures/NN/test_heatmap_using_relu.pdf')
