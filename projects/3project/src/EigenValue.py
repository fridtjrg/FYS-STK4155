...
class EigenValue:
    ...
    def plot(self):
        ...
        ax0.plot(range(self.num_iter+1), self.EV[:, 0], color='b', label=f'NN $v_1$={self.x_dnn[0][0]:.5f}')
        ax0.plot(range(self.num_iter+1), self.EV[:, 1], color='g', label=f'NN $v_2$={self.x_dnn[1][0]:.5f}')
        ax0.plot(range(self.num_iter+1), self.EV[:, 2], color='r', label=f'NN $v_3$={self.x_dnn[2][0]:.5f}')
        ax0.plot(range(self.num_iter+1), self.EV[:, 3], color='y', label=f'NN $v_4$={self.x_dnn[3][0]:.5f}')
        ax0.plot(range(self.num_iter+1), self.EV[:, 4], color='k', label=f'NN $v_5$={self.x_dnn[4][0]:.5f}')
        ax0.plot(range(self.num_iter+1), self.EV[:, 5], color='c', label=f'NN $v_6$={self.x_dnn[5][0]:.5f}')
